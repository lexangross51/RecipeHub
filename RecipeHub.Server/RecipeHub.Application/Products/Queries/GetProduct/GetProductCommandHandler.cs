...
internal class GetProductCommandHandler(IProductRepository repos, IValidator<GetProductQuery> validator, IMapper mapper) 
    : RequestHandler<GetProductQuery, ProductDto>(validator)
{
    protected override async Task<Result<ProductDto>> HandleAsync(GetProductQuery request, CancellationToken cancellationToken)
    {
        ...
        if (product == null)
        {
            ...
            return Result.Fail<ProductDto>(errorMessage);
        }
